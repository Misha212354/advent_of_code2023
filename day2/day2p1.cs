    ...
    class Day2P1
    {
        ...
        static void Main()
        {
            ...
            using (FileStream fileStream = new FileStream(filePath, FileMode.Open, FileAccess.Read))
            {
                ...
                {
                    // parse each line out
                    // find the largest number presented for red, green, and blue in each game
                    //  compare if less then  12 red, 13 green, 14 blue
                    //  if less of equal to 
                    //  take id and add to the sum
                    // the first digit is always game number
                    // then if digit move one more check letter
                    
                    if((char)byteRead == '\n')
                    {
                        if(currRed <= 12 && currGreen <= 13 && currBlue <= 14)
                        {
